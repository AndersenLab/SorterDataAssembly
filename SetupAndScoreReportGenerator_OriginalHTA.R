...
file.fxns <- "SorterDataAssembly/ProcessSorterFxns_Princeton_TCS.R"
...
file.strains.ctrl <- "ctrlstrains.Rds"
file.strains.pq <- "pqstrains.Rds"
...
##########################
load("~/Downloads/allstrains.RData")
ctrlStrains <- list(ctrl.strains_a, ctrl.strains_b, ctrl.strains_c, ctrl.strains_d)
pqStrains <- list(pq.strains_a, pq.strains_b, pq.strains_c, pq.strains_d)

completeDFs <- list()

for(dir in seq(1,length(dataDirs))){
    dir.data <- dataDirs[dir]
    controlStrains <- ctrlStrains[[dir]]
    paraquatStrains <- pqStrains[[dir]]
...
#         print(paste0("Done with setup #", i))
    ...
    for(i in 1:length(score.modplate)){
        if(grepl("control", score.filelist[[i]][1])){
            ...
            print(pqPlate)
            strains = paraquatStrains[[pqPlate]]
            strains<-matrix(strains,nrow=8,ncol=12,byrow=TRUE)
            plate = summarizePlate_worms(score.modplate[[i]], t(strains), quantiles=TRUE, log=TRUE)
            score.pheno = append(score.pheno, list(plate))
            pqPlate = pqPlate + 1
...
#     
    ...
    for(i in 1:(length(score.pheno)))
    {
        ...
        saveRDS(score.pheno[[i]]$strains,file=file.path(dir.existing,"temp","strains.rds"))
...
for(dir in seq(1,length(dataDirs))){
    dir.data <- dataDirs[dir]
    data <- assays[[dir]]
    controlPlates = unique(data[data$drug == "control", which(colnames(data) == "plate")])
    testPlates = unique(data[data$drug == "paraquat", which(colnames(data) == "plate")])
    ...
    controls <- controlPlates
    means <- list()
    length(means) = ncol(data)
    plates <- data[data$plate %in% controls,]
    meanPlate = plates %>% group_by(strain) %>% do(data.frame(means = colMeans(.[10:ncol(.)])))
    meanPlate = as.data.frame(cbind(as.character(unique(meanPlate$strain)), matrix(meanPlate$means, ncol=ncol(plates)-9, byrow=TRUE)))
    colnames(meanPlate)[1] = "strain"
    controlPlate = as.data.frame(cbind(data.frame(matrix(nrow=nrow(meanPlate), ncol=8)), meanPlate))
    controlPlate[is.na(controlPlate$strain),] = NA
    
    for(j in controlPlates){
...
controlsData = data.frame(do.call(rbind, contData))

plateData[is.na(plateData$strain), 10:ncol(plateData)] = NA
controlsData$row = rep(LETTERS[1:8], each=12)
controlsData$col = rep(1:12)
controlsData[controlsData$col %% 4 == 0 | is.na(controlsData$col), 10:ncol(controlsData)] = NA
